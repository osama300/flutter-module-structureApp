...
class ApiManager {
  ...
  Future<ApiCallResponse> makeApiCall({
    ...
    try {
      ...
      final endTime = DateTime.now(); // End timing the request in case of error
      if(kDebugMode) {
        logApiCallDetails(
          callName: callName,
          url: apiUrl,
  ...
  void logApiCallDetails({
    ...
    try {
      ...
      log("│ contentLength: ${response!.request!.contentLength}");
      log("│ persistentConnection: ${response!.request!.persistentConnection}");
